//audio
datablock AudioProfile(AdventureThompsonFireSound)
{
   filename    = "./Sounds/AdventureThompson_Fire.wav";
   description = AudioDefault3d;
   preload = true;
};


AddDamageType("AdventureThompson",   '<bitmap:add-ons/Weapon_Expansion_TTOverhaul_Adventure/Icons/CI_AdventureThompson> %1 Didn\'t even try with the Thompson SMG',    '%2 Made <bitmap:add-ons/Weapon_Expansion_TTOverhaul_Adventure/Icons/CI_AdventureThompson> %1 kick their bucket',0.75,1);
datablock ProjectileData(AdventureThompsonProjectile : TTOBulletProjectile)
{
   directDamage        = 18;
   directDamageType    = $DamageType::AdventureThompson;
   radiusDamageType    = $DamageType::AdventureThompson;
...
// item //
//////////
datablock ItemData(AdventureThompsonItem)
{
	category = "Weapon";  // Mission editor category
	className = "Weapon"; // For inventory system

	 // Basic Item Properties
	shapeFile = "./Models/AdventureThompson.dts";
	rotate = false;
	mass = 1;
	density = 0.2;
	elasticity = 0.2;
	...
	//gui stuff
	uiName = "Thompson SMG";
	iconName = "./Icons/AdventureThompson";
	doColorShift = true;
	colorShiftColor = "0.6 0.6 0.67 1.000";

	 // Dynamic properties defined by the scripts
	image = AdventureThompsonImage;
	canDrop = true;
...
//weapon image//
////////////////
datablock ShapeBaseImageData(AdventureThompsonImage)
{
   // Basic Item properties
   shapeFile = "./Models/AdventureThompson.dts";
   emap = true;
   ...
   // Projectile && Ammo.
   item = AdventureThompsonItem;
   ammo = " ";
   projectile = AdventureThompsonProjectile;
   ...
   doColorShift = true;
   colorShiftColor = AdventureThompsonItem.colorShiftColor;
...
function AdventureThompsonImage::onFire(%this,%obj,%slot)
{

	%obj.stopAudio(1);
	%obj.playAudio(1, AdventureThompsonFireSound);
...
function AdventureThompsonImage::onReloadStart(%this,%obj,%slot)
{
...
function AdventureThompsonImage::onReloadWait(%this,%obj,%slot)
{
	if($Pref::Server::TTO::DeathStopAnims && %obj.getDamagePercent() >= 1.0)
		return;
	%this.TTO_displayAmmo(%obj);
...
function AdventureThompsonImage::onReloaded(%this,%obj,%slot)
{
	if($Pref::Server::TTO::DeathStopAnims && %obj.getDamagePercent() >= 1.0)
...
function AdventureThompsonProjectile::damage(%this,%obj,%col,%fade,%pos,%normal)
{
